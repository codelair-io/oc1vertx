...
async def run_load_test():
  url = "http://localhost:8080/work?input={}"
  tasks = []
  async with ClientSession() as session:
    for j in range(10000):
      letters = ''.join(random.choice(string.ascii_letters) for v in range(10))
      task = asyncio.ensure_future(send_request(url.format(letters), session))
      tasks.append(task)
...
filename = target + '_busywork_load_test_' + uname.system + '_' + uname.machine + '.txt'
